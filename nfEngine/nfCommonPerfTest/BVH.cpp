...
void QueryCallback(void* leafUserData, void* userData)
{
    (void)leafUserData;
    (void)userData;
}
...
TEST(BVH, InsertAndRemove)
{
    ...
    std::cout << "Leaves   | Insert time [ms] | Query time [us] | Big query time [us] |Remove time [ms]" << std::endl;
    for (int leavesNum = leavesNumStart; leavesNum < leavesNumEnd; leavesNum *= 2)
    {
        ...
        for (int i = 0; i < queriesNum; ++i)
        {
            ...
            bvh.Query(box, QueryCallback, nullptr);
        }
