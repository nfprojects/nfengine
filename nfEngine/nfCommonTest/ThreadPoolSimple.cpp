...
TEST(ThreadPoolSimple, EmptyTask)
{
    ...
    ASSERT_NO_THROW(tp.CreateAndEnqueueTask(desc));
}
...
TEST(ThreadPoolSimple, EnqueueInsideTask)
{
    ...
    for (int i = 0; i < 10000; ++i)
    {
        ...
        TaskFunction taskFuncA = [&](const TaskContext& context)
        {
            ...
            desc.waitable = true;
            ASSERT_NO_THROW(task = tp.CreateAndEnqueueTask(desc));
        };
...
TEST(ThreadPoolSimple, Instances)
{
    ...
    for (int i = 0; i < tasksNum; ++i)
    {
        ...
        TaskDesc desc;
        desc.function = std::bind(func, _1, i);
...
TEST(ThreadPoolSimple, DependencyChain)
{
    ...
    TaskDesc desc;
    desc.function = std::bind(func, _1, 0);
...
TEST(ThreadPoolSimple, EnqueueChildRecursive)
{
    ...
    [&](const TaskContext& context, int depth)
    {
        ...
        {
            TaskDesc desc;
            desc.function = std::bind(taskFunc, _1, depth + 1);
            desc.parent = context.taskId;
    ...
    // spawn root task
    TaskDesc desc;
    desc.function = std::bind(taskFunc, _1, 0);
    desc.waitable = true;
