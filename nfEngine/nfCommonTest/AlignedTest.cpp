...
class TestClass16 : public Aligned<16>
{
    int a;
};

class TestClass64 : public Aligned<64>
{
    int a;
};

class TestClass4096 : public Aligned<4096>
{
    int a;
};
...
TEST(Aligned, AlignedArray)
{
    ...
    std::unique_ptr<TestClass16[]> testArray16(new TestClass16[ARRAY_SIZE]);
    std::unique_ptr<TestClass64[]> testArray64(new TestClass64[ARRAY_SIZE]);
    std::unique_ptr<TestClass4096[]> testArray4096(new TestClass4096[ARRAY_SIZE]);

    EXPECT_EQ(0, reinterpret_cast<size_t>(testArray16.get()) % 16);
    EXPECT_EQ(0, reinterpret_cast<size_t>(testArray64.get()) % 64);
    EXPECT_EQ(0, reinterpret_cast<size_t>(testArray4096.get()) % 4096);
}
