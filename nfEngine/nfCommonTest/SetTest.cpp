...
TEST(Set, InsertViaMove)
{
...

    {
        ...
        ASSERT_NE(set.End(), set.Insert(std::move(Type(&counters, 1))).iterator);
        ASSERT_NE(set.End(), set.Insert(std::move(Type(&counters, 2))).iterator);
        ASSERT_EQ(2, set.Size());
        // set = [1, 2]
...
TEST(Set, MoveAssignment)
{
    ...
    ClassMethodCallCounters counters;
    {
        ...
        // initialize source set
        SetType set2;
        ASSERT_NE(set2.End(), set2.Insert(std::move(Type(&counters, 3))).iterator);
        ASSERT_NE(set2.End(), set2.Insert(std::move(Type(&counters, 4))).iterator);
        // set = [1, 2]
        // set2 = [3, 4]
        EXPECT_EQ(4, counters.constructor);
