...
TEST(ReflectionClassTest, TestClassWithNestedType_Verify)
{
    ...
    ASSERT_EQ(2, members.size());
    {
        ...
        EXPECT_EQ(12, members[1].GetOffset());
    }
...
TEST(ReflectionClassTest, TestClassWithNestedType_Serialization)
{
    ...
    const char* REFERENCE_CONFIG_STRING = "obj={mFoo={mInteger=789 mFloat=16 mBool=false} mBar=123}";
    EXPECT_EQ(REFERENCE_CONFIG_STRING, config.ToString(false));
}