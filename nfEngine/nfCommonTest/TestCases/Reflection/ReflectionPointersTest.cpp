...
using TestUniquePtr = std::unique_ptr<TestBaseClass>;
using TestSharedPtr = std::shared_ptr<TestBaseClass>;
...
TEST(ReflectionClassTest, UniquePtr_Verify)
{
    ...
    EXPECT_STREQ("std::unique_ptr<TestBaseClass>", type->GetName());
...
TEST(ReflectionClassTest, UniquePtr_Serialize_Base)
{
    ...
    TestUniquePtr obj = std::make_unique<TestBaseClass>();
    obj->floatVal = 123.0f;
    obj->intVal = 321;
