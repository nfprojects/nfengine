...
TEST(Math, Sin)
{
    ...
    TestTranscendental("Sin", range, Math::Sin, sinf, 5.0e-07f, 1.0f);
}
...
{
    TestRange range(-5.0f, 5.0f, 0.01f, TestRange::StepType::Increment);
    TestTranscendental("Exp", range, Math::Exp, expf, 1.0f, 3.0e-07f);
}
...
TEST(Math, Transcendental_Perf)
{
    const uint32 numSamples = 10000000;
    std::vector<float> ref, calc;
    ref.resize(numSamples);
    calc.resize(numSamples);
    ...
    NFE_LOG_INFO("log: %f ms", 1000.0f * timer.Stop());
    ...
    NFE_LOG_INFO("PERF TEST: %f ms", 1000.0f * timer.Stop());
}
