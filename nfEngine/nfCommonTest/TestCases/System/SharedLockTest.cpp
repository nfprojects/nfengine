...
{
    const uint32 maxIterations = 100000u;

    alignas(64) TypeParam lock; // for synchronizing counter access

    alignas(64) std::atomic<uint32> iteration = 0;

    alignas(64) std::atomic<uint32> numReaders = 0;
    alignas(64) std::atomic<uint32> numWriters = 0;
    ...
    const size_t numReaderThreads = std::min<size_t>(1024, std::thread::hardware_concurrency() / 2);
    const size_t numWriterThreads = std::min<size_t>(1024, std::thread::hardware_concurrency() / 2);
