...
{
public:
    TestDerivedClass(int& counter) : TestClass(counter) { }
    int mFoo;
};
...
TEST(SharedPtr, StaticCast)
{
    int counter = 0;

    SharedPtr<TestDerivedClass> pointerToDerived(new TestDerivedClass(counter));
    ASSERT_EQ(0, counter);

    SharedPtr<TestClass> pointerToBase = StaticCast<TestClass>(pointerToDerived);
    ASSERT_EQ(0, counter);

    pointerToDerived.Reset();
    ASSERT_EQ(0, counter);
