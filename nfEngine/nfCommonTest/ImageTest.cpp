...
protected:
    ...
    void FillTestImage(ImageFormat textureFormat)
    {
        ...
        {
            const int testSquareSize = 16 * 16;
            const int testSquareLineSize = 16;
            ...
            if (textureFormat == ImageFormat::A_UByte)
            {
                ...
                mTestImageA.get()->SetData(testSquare, width, height, textureFormat);
            }
            else
            {
                mTestImageR.reset(new Image());
                mTestImageR.get()->SetData(testSquare, width, height, textureFormat);
            }
        ...
        else
        {
            ...
            mTestImageRGB.get()->SetData(testSquare, width, height, textureFormat);
        }
    }
    ...
    void CheckTexels(Image* img)
    {
        ...
        for (int i = 0; i < TEXTURE_HEIGHT; i++)
        {
            for (int j = 0; j < TEXTURE_WIDTH; j++)
            {
                ...
                {
                    diff = (testTexelUCh[m] > texelUCh[m] ?
