...
{
    Image mImage_copy1;
    mImage_copy1.SetData(const_cast<char*>(TEXT.data()), 5, 5, ImageFormat::BC1);
    Image mImage_copy2(mImage_copy1);

    ASSERT_EQ(1, mImage_copy2.GetMipmapsNum());
    ASSERT_EQ(const_cast<char*>(TEXT.data()), mImage_copy2.GetData());
    ASSERT_EQ(ImageFormat::BC1, mImage_copy2.GetFormat());
}
...
TEST_F(ImageTest, Load)
{
    ...
    // PNG
    texture = new FileInputStream(texturePNG.data());
    LoadAssert();
...
TEST_F(ImageTest, FormatToString)
{
    std::string format = Image::FormatToStr(ImageFormat::R_UByte);
    ASSERT_EQ(0, format.compare("R_uchar"));
    format = Image::FormatToStr(ImageFormat::Unknown);
    ASSERT_EQ(0, format.compare("Unknown"));
}
