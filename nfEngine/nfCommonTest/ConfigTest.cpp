...
TEST(Config, ValueCheck)
{
    ...
    auto arrayIteratorCallback = [&] (const ConfigValue& value)
    {
        (void)value;
        return true;
    };
...
TEST(Config, Generate)
{
    ...
    config.AddValue(root, "object", ConfigValue(subObject));


    ConfigObject subObject2;
    config.AddValue(subObject2, "blah2", ConfigValue(234));
    config.AddValue(root, "object2", ConfigValue(subObject2));
