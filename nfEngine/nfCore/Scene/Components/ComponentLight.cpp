...
void LightComponent::OnDetach()
{
    ...
    {
        GetScene()->GetRendererSystem()->DeleteLightProxy(mRenderingProxy);
        mRenderingProxy = INVALID_RENDER_PROXY;
    }
}
...
void LightComponent::CreateRenderingProxy(bool update)
{
    ...
    if (update)
    {
        ...
        scene->GetRendererSystem()->UpdateLightProxy(mRenderingProxy, proxy);
    }
    else
    {
        mRenderingProxy = scene->GetRendererSystem()->CreateLightProxy(proxy);
    }
}
