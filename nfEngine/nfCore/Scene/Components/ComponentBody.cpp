...
void BodyComponent::OnBodyProxyMoved(const Math::Matrix& newTransform)
{
...
void BodyComponent::CreatePhysicsProxy()
{
    ...
    const auto updateCallback = [this](const Math::Matrix& newTransform)
    {
        OnBodyProxyMoved(newTransform);
    };
    ...
    proxyInfo.updateCallback = updateCallback;
