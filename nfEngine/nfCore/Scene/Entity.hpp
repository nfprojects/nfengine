...
/**
 * Scene Entity - spawned scene node. May be created dynamically or may come from a game object.
 *
 * @note
 * All const public methods are thread-safe and can be called at any moment.
 * All non-const public methods can be safely called only in two scenarios:
 * 1. From main thread, outside engine update routine.
 * 2. From owning entity controller's OnUpdate method.
 */
class CORE_API NFE_ALIGN16 Entity final
