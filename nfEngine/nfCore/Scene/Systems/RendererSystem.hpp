...
struct NFE_ALIGN16 LightProxy
{
    ...
    std::unique_ptr<Renderer::IResourceBindingInstance> lightMapBindingInstance;
...
public:
    ...
    RenderProxyID CreateMeshProxy(const MeshProxyDesc& data);
    RenderProxyID CreateLightProxy(const LightProxyDesc& data);
    bool UpdateMeshProxy(const RenderProxyID proxyID, const MeshProxyDesc& data);
    bool CreateLightProxy(const RenderProxyID proxyID, const LightProxyDesc& data);
