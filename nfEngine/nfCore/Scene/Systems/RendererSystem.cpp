...
void RendererSystem::RenderLights(const Common::TaskContext& context, RenderingData& data) const
{
    ...
    for (const LightProxy* lightProxy : data.visibleSpotLights)
    {
        ...
        LightsRenderer::Get()->DrawSpotLight(renderCtx->lightsContext.get(), prop, lightProxy->shadowMap.get(),
                                             lightProxy->lightMapBindingInstance.get());
    }
...
RenderProxyID RendererSystem::CreateMeshProxy(const MeshProxyDesc& data)
{
    return INVALID_RENDER_PROXY;
}

RenderProxyID RendererSystem::CreateLightProxy(const LightProxyDesc& data)
{
    return INVALID_RENDER_PROXY;
}
...
bool RendererSystem::CreateLightProxy(const RenderProxyID proxyID, const LightProxyDesc& data)
{
    return false;
}
