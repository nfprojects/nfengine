...
bool MultiShaderProgram::Load(const char* name)
{
    ...
    std::vector<char> str;
    {
        File file(gShadersProgramRoot + name + ".json", AccessMode::Read);
    ...
    auto parserCallback = [&](const char* key, const ConfigValue& value) -> bool
    {
        // read shader names
        for (int i = 0; i < NFE_SHADER_TYPES_NUM; ++i)
        {
            if (strcmp(key, gShaderNameStrings[i]) == 0)
            {
                const char* shaderName = value.GetString();
