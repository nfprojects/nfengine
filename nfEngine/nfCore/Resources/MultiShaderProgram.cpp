...
void MultiShaderProgram::OnUnload()
{
    for (int i = 0; i < NFE_SHADER_TYPES_NUM; ++i)
        mShaderResources[i].reset();
}

bool MultiShaderProgram::OnLoad()
{
    ResManager* rm = Engine::GetInstance()->GetResManager();
    LOG_INFO("Loading multi shader program '%s'...", mName);

    /// read JSON file
    std::vector<char> str;
    {
        Common::File file(gShadersProgramRoot + mName + ".json", Common::AccessMode::Read);
    ...
    {
        LOG_ERROR("Failed to parse multi shader program file '%s'...", mName);
        return false;
    }
...
bool MultiShaderProgram::GenerateShaderPrograms()
{
    ...
    for (size_t i = 0; i < NFE_SHADER_TYPES_NUM; ++i)
    {
        ...
        if (shader)
        {
            mShaderMacroMapping[i].reset(new int[mMacros.size()]);
...
IShader* MultiShaderProgram::GetShader(int* values, ShaderType type) const
{
    return nullptr;
}
