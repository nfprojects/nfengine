...
LightsRenderer::LightsRenderer()
{
     ...
     */
    const float phi = 1.6180345f;  // = golden_ratio = (sqrt(5) + 1) / 2
                           ...
                           const CameraRenderDesc* camera)
{
    ...
    Matrix viewProjMatrix = camera->viewMatrix * camera->projMatrix;
    ...
    cbuffer.projInverseMatrix = MatrixInverse(camera->projMatrix);
    cbuffer.viewProjMatrix = viewProjMatrix;
    cbuffer.viewProjInverseMatrix = viewProjInverse;
