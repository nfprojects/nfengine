...
void RenderCommandBuffer::Sort()
{
    ...
    int* pPointers = (int*)NFE_GENERIC_MALLOC(sizeof(int) * size, 1); // TODO avoid per-frame allocations
    for (size_t i = 0; i < size; i++)
        pPointers[i]  = static_cast<int>(i); // assume there is less than 2^32 items

    int* pTmpPointers = (int*)NFE_GENERIC_MALLOC(sizeof(int) * size, 1);

    merge_sort<int>(pPointers, size, pTmpPointers, RenderCommandCmp, &commands[0]);

    NFE_GENERIC_FREE(pTmpPointers);

    //reorganize
    RenderCommand* pTmpCommands = (RenderCommand*)NFE_GENERIC_MALLOC(sizeof(RenderCommand) * size, 16);
    memcpy(pTmpCommands, &commands[0], sizeof(RenderCommand) * size );
    for (size_t i = 0; i < commands.size(); i++)
    {
        commands[i] = pTmpCommands[pPointers[i]];
    }
    NFE_GENERIC_FREE(pTmpCommands);

    NFE_GENERIC_FREE(pPointers);
}
