...
void RendererSystem::Render(Renderer::View* view)
{
    ...
    TransformComponent* cameraTransform = nullptr; // TODO
    ...
    // TODO: velocity
    camera->Update(cameraTransform->GetMatrix(), Vector(), Vector(), 0.0f);
    ...
    CameraRenderDesc cameraRenderDesc =
    {
        ...
        Vector(), Vector(), // TODO: read velocities from body component
        camera->mScreenScale,
    ...
    for (auto lightTuple : mLights)
    {
        ...
        light->Update(camera);
        light->CheckShadowVisibility(cameraTransform->GetPosition());
