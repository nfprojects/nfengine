                     ...
                     const UpdateRequest* pUpdateRequests, uint32 updateRequestsNum)
{
    ...
    for (uint32 i = 0; i < drawRequestsNum; i++)
    {
        ...
        GuiRenderer::Get()->Enter(pCtx);
        {
            Recti rect;
            GuiRenderer::Get()->SetTarget(pCtx, pView->RT);
            pView->OnPostRender(pCtx, GuiRenderer::Get());

            //draw sign
            rect = Recti(6, 6, 100, 100);
            GuiRenderer::Get()->PrintTextWithBorder(pCtx, "NFEngine v0.1 [Demo]", rect, 0, 0, 0xFFFFFFFF,
                                                0x60000000);
        }
        GuiRenderer::Get()->Leave(pCtx);
