                     ...
                     const UpdateRequest* pUpdateRequests, uint32 updateRequestsNum)
{
    ...
    if (!(NFE::Common::MemoryCheck(pDrawRequests, sizeof(DrawRequest)*drawRequestsNum) & ACCESS_READ))
    {
    ...
    if (!(NFE::Common::MemoryCheck(pUpdateRequests, sizeof(UpdateRequest)*updateRequestsNum) & ACCESS_READ))
    {
    ...
    for (uint32 i = 0; i < drawRequestsNum; i++)
    {
        ...
        g_pGuiRenderer->Enter(pCtx);
        {
            ...
            g_pGuiRenderer->PrintTextWithBorder(pCtx, "NFEngine v0.1 [Demo]", rect, 0, 0, 0xFFFFFFFF,
                                                0x60000000);
        }
        g_pGuiRenderer->Leave(pCtx);
