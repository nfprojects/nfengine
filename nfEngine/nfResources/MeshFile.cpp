...
bool MeshFile::Save(Common::OutputStream& stream) const
{
    ...
    if (!stream.Write(&header, sizeof(header)))
        return false;
    if (!stream.Write(mVertices.data(), sizeof(MeshVertex) * header.verticesCount))
        return false;
    if (!stream.Write(mIndices.data(), sizeof(uint32) * header.indicesCount))
        return false;
    if (!stream.Write(mSubMeshes.data(), sizeof(MeshSubMesh) * header.subMeshesCount))
        return false;
