...
TEST_F(ProfilerTest, Stats)
{
...

    {
        ...
        for (unsigned int i = 0; i < VISIT_COUNT_CHILD3; ++i)
        {
    ...
    EXPECT_NEAR(rootStats.time, static_cast<double>(WAIT_TIME_SUM) / 1000.0, 0.01);
    EXPECT_EQ(rootStats.visitCount, 1);

    const ProfilerNodeStats& child1Stats = Child1->GetStats();
    EXPECT_NEAR(child1Stats.time,
                static_cast<double>(WAIT_TIME_CHILD1 + WAIT_TIME_CHILD1CHILD) / 1000.0, 0.002);
    EXPECT_EQ(child1Stats.visitCount, 1);

    const ProfilerNodeStats& child1childStats = Child1Child->GetStats();
    EXPECT_NEAR(child1childStats.time, static_cast<double>(WAIT_TIME_CHILD1CHILD) / 1000.0, 0.002);
    EXPECT_EQ(child1childStats.visitCount, 1);

    const ProfilerNodeStats& child2Stats = Child2->GetStats();
    EXPECT_NEAR(child2Stats.time, static_cast<double>(WAIT_TIME_CHILD2) / 1000.0, 0.002);
    EXPECT_EQ(child2Stats.visitCount, 1);

    const ProfilerNodeStats& child3Stats = Child3->GetStats();
    EXPECT_NEAR(child3Stats.time, static_cast<double>(WAIT_TIME_CHILD3 * VISIT_COUNT_CHILD3) / 1000.0, 0.005);
