...
const int WAIT_TIME_SUM = WAIT_TIME_ROOT + WAIT_TIME_CHILD1 + WAIT_TIME_CHILD1CHILD
                        + WAIT_TIME_CHILD2 + WAIT_TIME_CHILD3;
} // namespace
...
TEST_F(ProfilerTest, Scope)
{
    ...
    ASSERT_NEAR(stats.time, static_cast<double>(WAIT_TIME_CHILD1CHILD) / 1000.0, 0.001);
...
TEST_F(ProfilerTest, Stats)
{
    ...
    EXPECT_NEAR(rootStats.time, static_cast<double>(WAIT_TIME_SUM) / 1000.0, 0.003);
    EXPECT_EQ(rootStats.visitCount, 1);

    const ProfilerNodeStats& child1Stats = Child1->GetStats();
    EXPECT_NEAR(child1Stats.time,
                static_cast<double>(WAIT_TIME_CHILD1 + WAIT_TIME_CHILD1CHILD) / 1000.0, 0.001);
    EXPECT_EQ(child1Stats.visitCount, 1);

    const ProfilerNodeStats& child1childStats = Child1Child->GetStats();
    EXPECT_NEAR(child1childStats.time, static_cast<double>(WAIT_TIME_CHILD1CHILD) / 1000.0, 0.001);
    EXPECT_EQ(child1childStats.visitCount, 1);

    const ProfilerNodeStats& child2Stats = Child2->GetStats();
    EXPECT_NEAR(child2Stats.time, static_cast<double>(WAIT_TIME_CHILD2) / 1000.0, 0.001);
    EXPECT_EQ(child2Stats.visitCount, 1);

    const ProfilerNodeStats& child3Stats = Child3->GetStats();
    EXPECT_NEAR(child3Stats.time, static_cast<double>(WAIT_TIME_CHILD3) / 1000.0, 0.001);
    EXPECT_EQ(child3Stats.visitCount, 1);
}
