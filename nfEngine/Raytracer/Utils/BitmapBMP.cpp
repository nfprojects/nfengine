...
bool Bitmap::LoadBMP(FILE* file, const char* path)
{
    ...
    {
        NFE_LOG_ERROR("Failed to read BMP info header from file '%hs'", path);
        return false;
    }

    if (infoHeader.biPlanes != 1 || infoHeader.biCompression != 0)
    {
        NFE_LOG_ERROR("Unsupported BMP format: '%hs'", path);
        return false;
    }
    ...
    {
        NFE_LOG_ERROR("Unsupported BMP bit depth (%u): '%hs'", (uint32)infoHeader.biBitCount, path);
        return false;
    }

    if (infoHeader.biWidth == 0 || infoHeader.biHeight == 0)
    {
        NFE_LOG_ERROR("Invalid image size: '%hs'", path);
        return false;
    }
    ...
    if (paletteBytes > 0)
    {
        ...
        {
            NFE_LOG_ERROR("Failed to read bitmap palette from file '%hs'", path);
            return false;
        }
    }
    ...
    {
        NFE_LOG_ERROR("Failed to read bitmap data from file '%hs'", path);
        return false;
    }
...
bool Bitmap::SaveBMP(const char* path, bool flipVertically) const
{
    ...
    if (!file)
    {
        NFE_LOG_ERROR("Failed to open target image '%s', code = %u", path, stderr);
        return false;
    }

    if (fwrite(&header, sizeof(BMPHeader), 1, file) != 1)
    {
        NFE_LOG_ERROR("Failed to write bitmap header to file '%s', code = %u", path, stderr);
        fclose(file);
        return false;
    }
    ...
    {
        NFE_LOG_ERROR("Failed to write bitmap image data to file '%s', code = %u", path, stderr);
        fclose(file);
        return false;
    }
