...
bool Bitmap::LoadDDS(FILE* file, const char* path)
{
    ...
    else if (pf.flags & DDPF_FOURCC)
    {
        ...
        else if (MAKEFOURCC('D', 'X', '1', '0') == pf.fourCC)
        {
            ...
            {
                NFE_LOG_ERROR("Failed to read DX10 header '%hs'", path);
                return false;
            }
    ...
    {
        NFE_LOG_ERROR("Unsupported DDS format in file '%hs'", path);
        return false;
    }
    ...
    {
        NFE_LOG_ERROR("Failed to read bitmap data from file '%hs', errno = %u", path, errno);
        return false;
    }
