...
bool CommandListManager::OnFinishFrame()
{
    ...
    for (const CommandListPtr& commandList : mCommandLists)
    {
        if (commandList->GetState() == CommandList::State::Recorded)
        {
            LOG_ERROR("Command list (ID = %u) is in 'recorded' state. This is invalid - all command lists must be submitted before calling Device::FinishFrame()");
            success = false;
        }
        else if (commandList->GetState() == CommandList::State::Recording)
        {
            LOG_ERROR("Command list (ID = %u) is in 'recording' state. This is invalid - all command lists must be submitted before calling Device::FinishFrame()");
            success = false;
        }
    }
