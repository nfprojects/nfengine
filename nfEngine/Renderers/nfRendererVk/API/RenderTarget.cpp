...
bool RenderTarget::Init(Common::SharedPtr<Device>& device, const RenderTargetDesc& desc)
{
    mDevicePtr = device;
    ...
    mRenderPass = mDevicePtr->GetRenderPassManager()->GetRenderPass(rpDesc);
    ...
    else
    {
        ...
        fbInfo.pAttachments = fbAtts.Data();
        fbInfo.layers = 1;
        result = vkCreateFramebuffer(mDevicePtr->GetDevice(), &fbInfo, nullptr, &mFramebuffers[0]);
        VK_RETURN_FALSE_IF_FAILED(result, "Failed to create framebuffer");
    }
