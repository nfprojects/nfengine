...
Backbuffer::~Backbuffer()
{
    ...
    if (mSurface != VK_NULL_HANDLE)
        vkDestroySurfaceKHR(mDevicePtr->GetInstance(), mSurface, nullptr);
...
bool Backbuffer::SelectPresentQueue()
{
    ...
    uint32 queueCount;
    vkGetPhysicalDeviceQueueFamilyProperties(mDevicePtr->GetPhysicalDevice(), &queueCount, nullptr);
    if (queueCount == 0)
    {
    ...
    {
        vkGetPhysicalDeviceQueueFamilyProperties(mDevicePtr->GetPhysicalDevice(), &queueCount, queueProps.Data());
    }
    ...
    for (uint32 i = 0; i < queueCount; ++i)
    {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(mDevicePtr->GetPhysicalDevice(), i, mSurface,
                                                &supportsPresent);
        if (supportsPresent)
        {
...
bool Backbuffer::SelectSurfaceFormat(const BackbufferDesc& desc)
{
    ...
    VkResult result = vkGetPhysicalDeviceSurfaceFormatsKHR(mDevicePtr->GetPhysicalDevice(), mSurface,
                                                  &formatCount, nullptr);
    ...
    {
        result = vkGetPhysicalDeviceSurfaceFormatsKHR(mDevicePtr->GetPhysicalDevice(), mSurface,
...
bool Backbuffer::SelectPresentMode(const BackbufferDesc& desc)
{
    ...
    VkResult result = vkGetPhysicalDeviceSurfacePresentModesKHR(mDevicePtr->GetPhysicalDevice(), mSurface,
    ...
    {
        result = vkGetPhysicalDeviceSurfacePresentModesKHR(mDevicePtr->GetPhysicalDevice(), mSurface,
...
bool Backbuffer::SelectBufferCount()
{
    ...
    VkResult result = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(mDevicePtr->GetPhysicalDevice(), mSurface,
...
bool Backbuffer::CreateSwapchain(const BackbufferDesc& desc)
{
    ...
    VkResult result = vkCreateSwapchainKHR(mDevicePtr->GetDevice(), &swapInfo, nullptr, &mSwapchain);
...
bool Backbuffer::CreateSwapchainImageViews()
{
    ...
    VkResult result = vkGetSwapchainImagesKHR(mDevicePtr->GetDevice(), mSwapchain, &swapImageCount, nullptr);
    ...
    result = vkGetSwapchainImagesKHR(mDevicePtr->GetDevice(), mSwapchain, &mBuffersNum, images.Data());
    ...
    for (mCurrentBuffer = 0; mCurrentBuffer < mBuffersNum; mCurrentBuffer++)
    {
        ...
        result = vkCreateImageView(mDevicePtr->GetDevice(), &ivInfo, nullptr, &mImages[mCurrentBuffer].view);
...
bool Backbuffer::Init(Common::SharedPtr<Device>& device, const BackbufferDesc& desc)
{
    mDevicePtr = device;
