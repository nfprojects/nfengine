...
Texture::~Texture()
{
    ...
    for (auto imgMem: mImageMemories)
        if (imgMem != VK_NULL_HANDLE) 
            vkFreeMemory(gDevice->GetDevice(), imgMem, nullptr);

    for (auto imgView: mImageViews)
        if (imgView != VK_NULL_HANDLE)
            vkDestroyImageView(gDevice->GetDevice(), imgView, nullptr);

    if (!mFromSwapchain)
    {
        for (auto img: mImages)
            if (img != VK_NULL_HANDLE)
                vkDestroyImage(gDevice->GetDevice(), img, nullptr);
    }
}
...
bool Texture::Init(const TextureDesc& desc)
{
    ...
    mImageViews.resize(mBuffersNum);
    mImageMemories.resize(mBuffersNum);
    ...
    result = vkCreateImage(gDevice->GetDevice(), &imageInfo, nullptr, &mImages[0]);
    CHECK_VKRESULT(result, "Failed to create Image for texture");

    VkMemoryRequirements imageMemReqs;
    vkGetImageMemoryRequirements(gDevice->GetDevice(), mImages[0], &imageMemReqs);
    ...
    result = vkAllocateMemory(gDevice->GetDevice(), &imageMemInfo, nullptr, &mImageMemories[0]);
    CHECK_VKRESULT(result, "Failed to allocate memory for Image");

    result = vkBindImageMemory(gDevice->GetDevice(), mImages[0], mImageMemories[0], 0);
    ...
    if (desc.dataDesc)
    {
        ...
        vkCmdCopyBufferToImage(initCmdBuffer, stagingBuffer, mImages[0], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageCopyRegion);
    ...
    result = vkCreateImageView(gDevice->GetDevice(), &ivInfo, nullptr, &mImageViews[0]);
