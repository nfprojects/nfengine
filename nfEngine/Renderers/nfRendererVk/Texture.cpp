...
bool Texture::Init(const TextureDesc& desc)
{
    ...
    if (desc.dataDesc) imageInfo.initialLayout = mCurrentImageLayout = VK_IMAGE_LAYOUT_PREINITIALIZED;
    else imageInfo.initialLayout = mCurrentImageLayout = mImageLayout;
...
void Texture::Transition(VkCommandBuffer cmdBuffer, VkImageLayout targetLayout)
{
    if (targetLayout == VK_IMAGE_LAYOUT_UNDEFINED)
        targetLayout = mImageLayout;

    if (targetLayout == mCurrentImageLayout)
    ...
    imageBarrier.oldLayout = mCurrentImageLayout;
    ...
    mCurrentImageLayout = targetLayout;
}
