...
Buffer::~Buffer()
{
    ...
    if (mStagingBufferMemory != VK_NULL_HANDLE)
        vkFreeMemory(gDevice->GetDevice(), mStagingBufferMemory, nullptr);
    if (mStagingBuffer != VK_NULL_HANDLE)
        vkDestroyBuffer(gDevice->GetDevice(), mStagingBuffer, nullptr);
}
...
bool Buffer::Init(const BufferDesc& desc)
{
    ...
    VkResult result = vkCreateBuffer(gDevice->GetDevice(), &bufInfo, nullptr, &mStagingBuffer);
    CHECK_VKRESULT(result, "Failed to create staging buffer for data upload");

    VkMemoryRequirements stagingMemReqs;
    vkGetBufferMemoryRequirements(gDevice->GetDevice(), mStagingBuffer, &stagingMemReqs);
    ...
    result = vkAllocateMemory(gDevice->GetDevice(), &memInfo, nullptr, &mStagingBufferMemory);
    CHECK_VKRESULT(result, "Failed to allocate memory for staging buffer");

    void* bufferData = nullptr;
    result = vkMapMemory(gDevice->GetDevice(), mStagingBufferMemory, 0, memInfo.allocationSize, 0, &bufferData);
    CHECK_VKRESULT(result, "Failed to map staging buffer's memory");
    memcpy(bufferData, desc.initialData, desc.size);
    vkUnmapMemory(gDevice->GetDevice(), mStagingBufferMemory);
    result = vkBindBufferMemory(gDevice->GetDevice(), mStagingBuffer, mStagingBufferMemory, 0);
    ...
    memInfo.memoryTypeIndex = gDevice->GetMemoryTypeIndex(stagingMemReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    region.size = static_cast<VkDeviceSize>(desc.size);
    vkCmdCopyBuffer(copyCmdBuffer, mStagingBuffer, mBuffer, 1, &region);
