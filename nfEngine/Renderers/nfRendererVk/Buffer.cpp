...
{
    if (desc.mode == BufferMode::Dynamic || desc.mode == BufferMode::GPUOnly || desc.mode == BufferMode::Readback)
    {
    ...
    if (desc.type == BufferType::Constant)
    {
        ...
        if (desc.initialData)
        {
            ...
            result = vkMapMemory(gDevice->GetDevice(), mBufferMemory, 0, memInfo.allocationSize, 0, &bufferData);
        ...
        LOG_INFO("%u-byte Constant Buffer created successfully", desc.size);
        return true;
    }
