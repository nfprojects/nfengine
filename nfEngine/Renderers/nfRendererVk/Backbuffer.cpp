...
bool Backbuffer::CreateSwapchainImageViews()
{
    ...
    result = vkGetSwapchainImagesKHR(gDevice->GetDevice(), mSwapchain, &mBuffersNum, mImages.data());
    ...
    for (mCurrentBuffer = 0; mCurrentBuffer < mBuffersNum; mCurrentBuffer++)
    {
        ...
        ivInfo.image = mImages[mCurrentBuffer];
        ...
        result = vkCreateImageView(gDevice->GetDevice(), &ivInfo, nullptr, &mImageViews[mCurrentBuffer]);
...
bool Backbuffer::BuildPresentCommandBuffers()
{
    ...
    for (uint32 i = 0; i < mBuffersNum; ++i)
    {
        ...
        presentBarrier.image = mImages[i];
