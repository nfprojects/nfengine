...
bool Backbuffer::Init(const BackbufferDesc& desc)
{
    ...
    // Check if we can transform as we would like
    VkSurfaceTransformFlagBitsKHR transform;
    LOG_DEBUG("Device's supported Surface transforms: 0x%x", surfCaps.supportedTransforms);
    if (surfCaps.supportedTransforms & VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_BIT_KHR)
        transform = VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_BIT_KHR;
    else
        transform = surfCaps.currentTransform;
