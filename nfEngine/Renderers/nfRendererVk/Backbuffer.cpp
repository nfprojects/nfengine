...
bool Backbuffer::Init(const BackbufferDesc& desc)
{
    ...
    {
        LOG_ERROR("Incorrect amount of swap images created (%d, requested %d)", swapImageCount, mBuffersNum);
        return false;
    }

    result = vkGetSwapchainImagesKHR(gDevice->GetDevice(), mSwapchain, &swapImageCount, mBuffers);
    CHECK_VKRESULT(result, "Failed to get swapchain images");

    LOG_DEBUG("%d swapchain buffers", mBuffersNum);
