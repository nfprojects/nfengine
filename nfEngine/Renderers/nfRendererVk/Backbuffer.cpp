...
bool Backbuffer::Init(const BackbufferDesc& desc)
{
    ...
    mCurrentBuffer = 0;
    vkAcquireNextImageKHR(gDevice->GetDevice(), mSwapchain, UINT64_MAX, gDevice->GetPresentSemaphore(), VK_NULL_HANDLE, &mCurrentBuffer);
...
{
    // Before presenting, ensure the present queue is useable
    ...
    const VkSemaphore& presSem = gDevice->GetPresentSemaphore();
    submitInfo.pSignalSemaphores = &presSem;
