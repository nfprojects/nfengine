...
bool Backbuffer::GetSurfaceFormat(const BackbufferDesc& desc)
{
...
bool Backbuffer::SelectBufferCount()
{
    ...
    mBuffersNum = 2;
    if (mSurfCaps.maxImageCount > 0)
        if (mBuffersNum > mSurfCaps.maxImageCount)
        {
            LOG_WARNING("Requested %d swapchain image count exceeds max limit %d - reducing",
                        mBuffersNum, mSurfCaps.maxImageCount);
            mBuffersNum = mSurfCaps.maxImageCount;
        }

    if (mSurfCaps.minImageCount > 0)
        if (mBuffersNum < mSurfCaps.minImageCount)
        {
            LOG_WARNING("Requested %d swapchain image count exceeds minimum limit %d - increasing",
                        mBuffersNum, mSurfCaps.minImageCount);
            mBuffersNum = mSurfCaps.minImageCount;
        }
...
bool Backbuffer::CreateSwapchain(const BackbufferDesc& desc)
{
    ...
    swapInfo.preTransform = mSurfCaps.currentTransform;
...
bool Backbuffer::Init(const BackbufferDesc& desc)
{
    ...
    if (!GetSurfaceFormat(desc)) return false;
