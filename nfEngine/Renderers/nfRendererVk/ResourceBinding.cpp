...
bool ResourceBindingLayout::Init(const ResourceBindingLayoutDesc& desc)
{
    ...
    if (desc.numBindingSets + desc.numVolatileCBuffers > 0)
    {
        ...
        poolInfo.maxSets = static_cast<uint32>(desc.numBindingSets); // TODO take volatiles into account
        ...
        for (size_t i = 0; i < desc.numBindingSets; ++i)
        {
            ...
            VkDescriptorSet& set = rbs->mDescriptorSet;
            VkDescriptorSet* setptr = &set;
            result = vkAllocateDescriptorSets(gDevice->GetDevice(), &allocInfo, setptr);
