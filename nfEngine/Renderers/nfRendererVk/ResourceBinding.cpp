...
bool ResourceBindingLayout::Init(const ResourceBindingLayoutDesc& desc)
{
    ...
    VkResult result = vkCreateDescriptorSetLayout(gDevice->GetDevice(), &dslInfo, nullptr, &mVolatileBufferLayout);
    ...
    if (desc.numBindingSets + desc.numVolatileCBuffers > 0)
    {
        ...
        poolInfo.maxSets = static_cast<uint32>(desc.numBindingSets + 1); // TODO are we sure about this "1"?
        ...
        vkAllocateDescriptorSets(gDevice->GetDevice(), &allocInfo, &mVolatileBufferSet);
    }
