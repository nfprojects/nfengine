...
VkPipeline PipelineState::CreateFullPipelineState(const FullPipelineStateParts& parts, const VkRenderPass rp)
{
    ...
    if (ps->mDesc.blendState.independent)
    {
        // TODO
    }
    else
    {
        ...
        state.blendEnable = ps->mDesc.blendState.rtDescs[0].enable;
        state.srcColorBlendFactor = TranslateBlendFuncToVkBlendFactor(ps->mDesc.blendState.rtDescs[0].srcColorFunc);
        state.dstColorBlendFactor = TranslateBlendFuncToVkBlendFactor(ps->mDesc.blendState.rtDescs[0].destColorFunc);
        state.colorBlendOp = TranslateBlendOpToVkBlendOp(ps->mDesc.blendState.rtDescs[0].colorOperator);
        state.srcAlphaBlendFactor = TranslateBlendFuncToVkBlendFactor(ps->mDesc.blendState.rtDescs[0].srcAlphaFunc);
        state.dstAlphaBlendFactor = TranslateBlendFuncToVkBlendFactor(ps->mDesc.blendState.rtDescs[0].destAlphaFunc);
        state.colorBlendOp = TranslateBlendOpToVkBlendOp(ps->mDesc.blendState.rtDescs[0].alphaOperator);
        state.colorWriteMask = VK_COLOR_COMPONENT_FLAG_BITS_MAX_ENUM;
        states.push_back(state);
    }
    // TODO attachmentCount should equal color attachment count inside render pass!!
