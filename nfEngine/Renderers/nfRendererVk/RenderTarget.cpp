...
bool RenderTarget::Init(const RenderTargetDesc& desc)
{
    ...
    VkSampleCountFlagBits samplesNum = VK_SAMPLE_COUNT_1_BIT;
    ...
    if (mDepthTex)
    {
        ...
        samplesNum = mDepthTex->mSamplesNum;
    }
    else
    {
        // TODO can we assume that samples num will be equal through all color targets?
        samplesNum = mTex[0]->mSamplesNum;
    }

    RenderPassDesc rpDesc(colorFormats, desc.numTargets, depthFormat, samplesNum);
