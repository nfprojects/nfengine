...
class QueueManager
{
    ...
    struct Queue
    {
        ...
        VkHandle<VkCommandPool> commandPool;
...
public:
    ...
    NFE_INLINE VkQueue GetQueue(QueueType type)
    {
        return mQueues[type].queue;
    }

    NFE_INLINE uint32 GetQueueFamilyIndex(QueueType type)
    {
        return mQueues[type].queueFamilyIndex;
    }

    NFE_INLINE VkCommandPool GetCommandPool(QueueType type)
    {
