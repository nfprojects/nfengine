...
int RenderTarget::Resize(int newWidth, int newHeight)
{
    ...
    hr = D3D_CHECK(gDevice->mDevice->CreateRenderTargetView(mTexture.get(), NULL, &mRTV));
    if (FAILED(hr))
    {
        // TODO: loging
        return 1;
...
int RenderTarget::InitSwapChain(int width, int height, HWND window)
{
    ...
    hr = D3D_CHECK(gDevice->mDXGIFactory->CreateSwapChain(gDevice->mDevice.get(), &scd, &mSwapChain));
    if (FAILED(hr))
    {
        return 1;
    }
