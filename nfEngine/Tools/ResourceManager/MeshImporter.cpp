...
bool MeshImporter::ImportOBJ(const std::string& sourceFilePath, const std::string& targetFilePath)
{
    ...
    if (indicesCount == 0 || subMeshesCount == 0 || verticesCount == 0)
    {
        LOG_ERROR("Empty mesh, ignoring.");
        return false;
    }
    ...
    // read indices
    memcpy(mIndices.data(), model.getIndexBuffer(), indicesCount * sizeof(mIndices[0]));
