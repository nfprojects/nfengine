...
bool MeshImporter::ImportOBJ(const std::string& sourceFilePath, const std::string& targetFilePath)
{
    ...
    File outputFile;
    if (!outputFile.Open(targetFilePath, AccessMode::Write, true))
    {
        LOG_ERROR("Could not open output file '%s'!", targetFilePath.c_str());
        return false;
    }

    return Save(FileOutputStream(outputFile));
}
