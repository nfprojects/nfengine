...
bool RenderTargetsScene::CreateBasicResources()
{
    ...
    if (!mVSBindingInstance)
        return false;
    if (!mVSBindingInstance->WriteCBuffer(0, mConstantBuffer.get()))
        return false;
...
bool RenderTargetsScene::CreateRenderTarget(bool withDepthBuffer, bool multipleRT, bool withMSAA)
{
    ...
    // create and fill binding set instance
    {
        ...
        if (!mPSBindingInstancePrimary)
            return false;
        if (!mPSBindingInstancePrimary->WriteTexture(0, mRenderTargetTextures[0].get(),
                                                     mSampler.get()))
            return false;
    }
    ...
    if (multipleRT)
    {
        ...
        if (!mPSBindingInstanceSecondary)
            return false;
        if (!mPSBindingInstanceSecondary->WriteTexture(0, mRenderTargetTextures[1].get(),
                                                       mSampler.get()))
            return false;
    }
    ...
    if (withDepthBuffer)
    {
        ...
        if (!mPSBindingInstanceDepth)
            return false;
        if (!mPSBindingInstanceDepth->WriteTexture(0, mDepthBuffer.get(), mSampler.get()))
            return false;
    }
