...
bool BasicScene::CreateTexture()
{
    ...
    if (!mPSBindingInstance)
        return false;
    if (!mPSBindingInstance->WriteTexture(0, mTexture.get()))
        return false;
...
bool BasicScene::OnSwitchSubscene()
{
    ...
    if (mConstantBuffer)
        mCommandBuffer->SetBindingSet(0, mVSBindingInstance.get());

    if (mTexture)
        mCommandBuffer->SetBindingSet(1, mPSBindingInstance.get());
