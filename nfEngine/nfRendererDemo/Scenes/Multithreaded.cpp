    ...
    : Scene("Basic")
{
    ...
    RegisterSubScene(std::bind(&MultithreadedScene::CreateSubSceneNormal, this, BufferMode::Dynamic, 20), "20x20, dynamic cbuffer");
    ...
    RegisterSubScene(std::bind(&MultithreadedScene::CreateSubSceneNormal, this, BufferMode::Volatile, 20), "20x20, volatile cbuffer");
}
...
void MultithreadedScene::DrawTask(const Common::TaskContext& ctx, int i, int j)
{
    ...
    recorder->Clear(ClearFlagsColor, 1, nullptr, &color);
...
void MultithreadedScene::Draw(float dt)
{
    ...
    for (int i = 0; i < mGridSize; ++i)
    {
        for (int j = 0; j < mGridSize; ++j)
        {
            ...
            const auto task = std::bind(&MultithreadedScene::DrawTask, this, _1, i, j);
            mThreadPool.CreateTask(task);
        }
    }
