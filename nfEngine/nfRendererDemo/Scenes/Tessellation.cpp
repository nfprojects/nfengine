...
{
    String vsPath = gShaderPathPrefix + "TessellationVS" + gShaderPathExt;
    mVertexShader = CompileShader(vsPath.Str(), ShaderType::Vertex, nullptr, 0);
    if (!mVertexShader)
        return false;

    String hsPath = gShaderPathPrefix + "TessellationHS" + gShaderPathExt;
    mHullShader = CompileShader(hsPath.Str(), ShaderType::Hull, nullptr, 0);
    if (!mHullShader)
        return false;

    String dsPath = gShaderPathPrefix + "TessellationDS" + gShaderPathExt;
    mDomainShader = CompileShader(dsPath.Str(), ShaderType::Domain, nullptr, 0);
    if (!mDomainShader)
        return false;

    String psPath = gShaderPathPrefix + "TessellationPS" + gShaderPathExt;
