...
PackerReader::PackerReader(const String& filePath): mFileVersion(0)
{
    Init(filePath);
}

PackerResult PackerReader::Init(const String& filePath)
{
    mFilePath = filePath;
...
PackerResult PackerReader::GetFile(const String& vfsFilePath, Buffer& outputBuffer)
{
    if (mFileList.empty())
