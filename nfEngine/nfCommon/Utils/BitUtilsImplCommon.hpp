 ...
 * @brief  File with defined common bit operations
 */
#pragma once
...
template<>
inline uint64 BitUtils<uint64>::ReverseBits(uint64 x)
{
    return
...
template<>
inline uint32 BitUtils<uint32>::ReverseBits(uint32 x)
{
    return
...
template<>
inline uint16 BitUtils<uint16>::ReverseBits(uint16 x)
{
    return
...
template<>
inline uint8 BitUtils<uint8>::ReverseBits(uint8 x)
{
...
{
    const size_t maxLen = CHAR_BIT * sizeof(T);
