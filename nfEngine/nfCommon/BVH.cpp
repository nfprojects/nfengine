...
uint32 TREE_ROTATION_TRESHOLD = 1;
...
int32 BVH::Rebalance(int32 node)
{
    ...
    if (balance > maxImbalance) // Rotate C up
    {
        ...
        if (F.height > G.height)
        {
            ...
            A.child1 = iG;
            G.parent = iA;
            A.box = Box(B.box, G.box);
            C.box = Box(A.box, F.box);
        ...
        else
        {
            ...
            A.child1 = iF;
            F.parent = iA;
            A.box = Box(B.box, F.box);
            C.box = Box(A.box, G.box);
    ...
    else if (balance < -maxImbalance) // Rotate B up
    {
        ...
        if (D.height > E.height)
        {
            ...
            A.child0 = iE;
            E.parent = iA;
            A.box = Box(C.box, E.box);
            B.box = Box(A.box, D.box);
        ...
        else
        {
            ...
            A.child0 = iD;
            D.parent = iA;
            A.box = Box(C.box, D.box);
            B.box = Box(A.box, E.box);
