...
/// logging macros
#define LOG_ANY(type, format, ...)                                             \
do {                                                                           \
    NFE::Common::Logger* logger = NFE::Common::Logger::GetInstance();          \
    if (logger) logger->Log(type,  __FILE__, __LINE__, format, __VA_ARGS__);   \
} while (0)
...
#define LOG_INFO(format, ...)    LOG_ANY(NFE::Common::LogType::Info, format, __VA_ARGS__)
#define LOG_INFO_S(msg)          LOG_ANY_STREAM(NFE::Common::LogType::Info, msg)
#define LOG_SUCCESS(format, ...) LOG_ANY(NFE::Common::LogType::OK, format, __VA_ARGS__)
#define LOG_SUCCESS_S(msg)       LOG_ANY_STREAM(NFE::Common::LogType::OK, msg)
#define LOG_WARNING(format, ...) LOG_ANY(NFE::Common::LogType::Warning, format, __VA_ARGS__)
#define LOG_WARNING_S(msg)       LOG_ANY_STREAM(NFE::Common::LogType::Warning, msg)
#define LOG_ERROR(format, ...)   LOG_ANY(NFE::Common::LogType::Error, format, __VA_ARGS__)
#define LOG_ERROR_S(msg)         LOG_ANY_STREAM(NFE::Common::LogType::Error, msg)
#define LOG_FATAL(format, ...)   LOG_ANY(NFE::Common::LogType::Fatal, format, __VA_ARGS__)
