    ...
    mChunkSize(0)
{
    ...
    mChunks = new uint32[mChunkSize];
    memset(mChunks, 0, mChunkSize*sizeof(uint32));

    // copy data
    memcpy(mChunks, input.c_str(), input.size());
    ...
    uint64 bitDataLength = (input.size()*8) % (0xffffffffffffffff);
...
void MD5Hash::Calculate(const std::string& input)
{
    ...
    for (unsigned int i=0; i < data.mChunkSize; i+=16)
    {
        uint32 a = h[0];
        uint32 b = h[1];
        uint32 c = h[2];
    ...
    // TODO: in memory data is aligned byte-to-byte correctly. However when kept in uint32 we have
    //       Big/Little Endian difference. Question - is this shifting necessary for us?
    h[0] = ((h[0] & 0xFF000000) >> 24) | ((h[0] & 0x00FF0000) >>  8) |
         ((h[0] & 0x0000FF00) << 8)  | ((h[0] & 0x000000FF) << 24);
    h[1] = ((h[1] & 0xFF000000) >> 24) | ((h[1] & 0x00FF0000) >>  8) |
         ((h[1] & 0x0000FF00) << 8)  | ((h[1] & 0x000000FF) << 24);
    h[2] = ((h[2] & 0xFF000000) >> 24) | ((h[2] & 0x00FF0000) >>  8) |
         ((h[2] & 0x0000FF00) << 8)  | ((h[2] & 0x000000FF) << 24);
    h[3] = ((h[3] & 0xFF000000) >> 24) | ((h[3] & 0x00FF0000) >>  8) |
         ((h[3] & 0x0000FF00) << 8)  | ((h[3] & 0x000000FF) << 24);
}
...
{
    return ((this->h[0] == hash.h[0]) && (this->h[1] == hash.h[1]) && (this->h[2] == hash.h[2]) && (this->h[3] == hash.h[3]));
}
...
std::ostream& operator<<(std::ostream& os, const MD5Hash& hash)
{
    ...
    os << std::setfill('0');
    os << std::setw(8) << hash.h[0] << std::setw(8) << hash.h[1] << std::setw(8) << hash.h[2] << std::setw(8) << hash.h[3];
