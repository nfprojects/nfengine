...
{
    ConfigNode* retNode = nullptr;
    ConfigNode* prevNode = nullptr;
    int line, column;
    Token token;
    ...
    for (;;)
    {
        ...
        ConfigNode* node = AllocateObjectNode();
...
void ConfigParser::Iterate(const InteratorCallback& callback, const ConfigNode* node) const
{
    if (node == nullptr)
        node = GetRootNode();
...
void ConfigParser::DebugPrint(const ConfigNode* node, int indent) const
{
    while (node != nullptr)
    {
        // print indent
