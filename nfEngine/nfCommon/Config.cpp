...
void Config::ValueToString(std::string& outputStr, ConfigValuePtr valuePtr, int intend) const
{
    ...
    case ConfigValue::Type::Object:
        ...
        for (int i = 0; i < intend; ++i)
            outputStr += INTEND;
        outputStr += "{\n";
        ObjectToString(outputStr, value.object, intend + 1);
        for (int i = 0; i < intend; ++i)
            outputStr += INTEND;
        outputStr += "}";
        break;
    case ConfigValue::Type::Array:
        {
            ...
            while (arrayNodePtr != INVALID_NODE_PTR)
            {
                ...
                ValueToString(outputStr, arrayNode.valuePtr, intend + 1);
...
void Config::ObjectToString(std::string& outputStr, ConfigObjectNodePtr objectPtr,
                            int intend) const
{
    while (objectPtr != INVALID_NODE_PTR)
    {
        ...
        outputStr += " =";
        ValueToString(outputStr, objectNode.valuePtr, intend);
        outputStr += '\n';
...
{
    std::string str;
    ObjectToString(str, GetRootNode(), 0);
    str += '\n';
    return str;
}
