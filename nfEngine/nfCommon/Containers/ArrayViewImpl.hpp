...
{
    static_assert(std::is_same<std::remove_cv<ElementType>::type, ElementType2>::value,
...
template<typename ElementType>
template<typename ElementType2>
typename ArrayView<ElementType>& ArrayView<ElementType>::operator = (const ArrayView<ElementType2>& other)
{
    static_assert(std::is_same<std::remove_cv<ElementType>::type, ElementType2>::value,
                  "Only (non-const -> const) ArrayView element type conversion is supported");
...
{
    static_assert(std::is_same<std::remove_cv<ElementTypeA>::type, std::remove_cv<ElementTypeB>::type>::value,
