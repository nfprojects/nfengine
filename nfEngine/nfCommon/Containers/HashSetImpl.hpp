    ...
    : HashSet()
{
    ...
    for (const KeyType& key : other)
    {
        ...
        new (mKeys + insertResult.iterator.mElement) KeyType(key);
    }
...
typename HashSet<KeyType, HashFunction>::InsertResult HashSet<KeyType, HashFunction>::InsertOrReplace(const KeyType& key)
{
    ...
    const InsertResult result = InsertOrReplace_Internal(key);
    if (result.replaced)
    {
...
typename HashSet<KeyType, HashFunction>::InsertResult HashSet<KeyType, HashFunction>::InsertOrReplace(KeyType&& key)
{
    ...
    const auto result = InsertOrReplace_Internal(key);
    if (result.replaced)
    {
        // destroy the old object
        mKeys[result.iterator.mElement].~KeyType();
...
typename HashSet<KeyType, HashFunction>::InsertResult HashSet<KeyType, HashFunction>::Insert(const KeyType& key)
{
    ...
    if (result.replaced)
    {
        // replacing is not supported by this method
        return InsertResult(End());
    }
