...
template<typename KeyType, typename Comparator>
Set<KeyType, Comparator>::ConstIterator::ConstIterator()
    : mSet(nullptr)
    , mNode(InvalidID)
{ }
...
template<typename KeyType, typename Comparator>
Set<KeyType, Comparator>::Iterator::Iterator(const Set* set, int node)
    : ConstIterator(set, node)
{ }

template<typename KeyType, typename Comparator>
KeyType& Set<KeyType, Comparator>::Iterator::operator*() const
{
    NFE_ASSERT(this->mNode != InvalidID, "Trying to dereference 'end' iterator");
    return this->mSet->mKeys[this->mNode];
}
