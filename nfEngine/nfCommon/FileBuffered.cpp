    ...
    } else
    {
        ...
        if (leftSpace <= 0)
        {
            ...
            if (leftSpace < FILE_BUFFERED_BUFFER_SIZE)
            {
                ...
                size_t lastChunkDiff = FILE_BUFFERED_BUFFER_SIZE - leftSpace;

                // Move data accordingly and move buffer position counter
                memmove(mBuffer + lastChunkDiff, mBuffer, leftSpace);
                mBufferOccupied = lastChunkDiff;
...
size_t FileBuffered::Write(const void* data, size_t size)
{
    ...
    if (mLastOperation == AccessMode::Read)
        mBufferOccupied = FILE_BUFFERED_BUFFER_SIZE;
    else if (mLastOperation == AccessMode::No)
        mBufferOccupied = 0;
...
bool FileBuffered::Seek(int64 pos, SeekMode mode)
{
    ...
    if (mLastOperation == AccessMode::Read)
    {
        ...
        {
            mBufferOccupied = static_cast<size_t>(relativePos);
            return true;
...
int64 FileBuffered::GetPos() const
{
    ...
    if (mLastOperation == AccessMode::Write)
        return mFile.GetPos() + mBufferOccupied;

    // When EOF is reached, buffer is empty
    if (mEofReached)
        return mFile.GetPos();
...
{
    return (position >= 0 && position < FILE_BUFFERED_BUFFER_SIZE);
}
