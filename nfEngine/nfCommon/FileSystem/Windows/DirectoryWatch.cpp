...
bool DirectoryWatch::WatchPath(const StringView path, Event eventFilter)
{
    {
        ...
        if (it != mRequests.End())
        {
            ...
            if (it->second->filter != static_cast<int>(eventFilter))
            {
                ...
                if (ret == 0)
                {
                    NFE_LOG_ERROR("QueueUserAPC() failed for path '%.*s': %s", path.Length(), path.Data(), GetLastErrorString().Str());
                    return false;
                }
    ...
    {
        NFE_LOG_ERROR("CreateFile() failed for path '%.*s': %s", path.Length(), path.Data(), GetLastErrorString().Str());
        return false;
    }
    ...
    NFE_LOG_DEBUG("Directory watch for path '%.*s' added", path.Length(), path.Data());
    return true;
}
