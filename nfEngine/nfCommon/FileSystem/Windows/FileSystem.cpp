...
bool FileSystem::Move(const StringView srcPath, const StringView destPath)
{
    ...
    {
        NFE_LOG_ERROR("Failed to move file '%.*s' to '%s': %.*s", srcPath.Data(), srcPath.Length(), destPath.Data(), destPath.Length(), GetLastErrorString().Str());
        return false;
    }

    NFE_LOG_INFO("File '%.*s' moved to '%.*s'", srcPath.Data(), srcPath.Length(), destPath.Data(), destPath.Length());
    return true;
}
...
bool FileSystem::Iterate(const StringView path, const DirIterateCallback& callback)
{
    ...
    while (!directories.empty())
    {
        ...
        if (findHandle == INVALID_HANDLE_VALUE)
        {
            NFE_LOG_ERROR("FindFirstFile failed for path '%.*s': %s", path.Data(), path.Length(), GetLastErrorString().Str());
            return false;
        }
        ...
        {
            FindClose(findHandle);
            NFE_LOG_ERROR("FindNextFile failed for path '%.*s': %s", path.Data(), path.Length(), GetLastErrorString().Str());
            return false;
        }
