...
class NFCOMMON_API AsyncThreadPool final
{
    ...
    std::atomic<AsyncFuncID> mStarted;
    ...
    AsyncFunc* GetTask(const AsyncFuncID& ptr) const;
...
public:
     ...
     */
    bool IsTaskFinished(const AsyncFuncID& taskPtr);

    /**
     * Waits for an task to finish.
     */
    void WaitForTask(const AsyncFuncID& taskPtr);
