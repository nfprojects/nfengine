    ...
    namespace NFE {                                                                     \
        namespace RTTI {                                                                \
            ...
            {                                                                           \
                ...
                typeInfo.serializer.reset(new FundamentalTypeSerializer<T>);            \
...
{                                                                                                       \
    ...
    static_assert(std::rank<MemberType>::value <= 1, "Multi-dimensional arrays are not supported");     \
    /* array support */                                                                                 \
    const size_t arraySize = std::extent<MemberType>::value;                                            \
    using BaseMemberType = std::remove_all_extents<MemberType>::type;                                   \
    /* register the member */                                                                           \
    typeInfo.members.push_back(NFE::RTTI::Member(#memberName,                                           \
                                                 offsetof(ClassType, memberName),                       \
                                                 NFE_GET_TYPE(BaseMemberType),                          \
                                                 arraySize));                                           \
}
