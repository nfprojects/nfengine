...
/**
 * Workaround for built-in offsetof() function that does not work with non-standard-layout types.
 */
 /*
...
struct OffsetOfImpl
{
    ...
    {
        return reinterpret_cast<size_t>(&(OffsetOfImpl<T1, T2>::object.*member)) -
               reinterpret_cast<size_t>(&OffsetOfImpl<T1, T2>::object);
    }
};
*/
/*
template <typename T1, typename T2>
T2 OffsetOfImpl<T1, T2>::object;
*/
/**
...
{
    //return OffsetOfImpl<T1, T2>::offset(member);
    const size_t fakePointer = 0x10;
