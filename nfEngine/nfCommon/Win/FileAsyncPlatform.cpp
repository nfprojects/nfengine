...
HANDLE FileAsync::mCallbackThread = nullptr;
HANDLE FileAsync::mDecayCallbackStart = ::CreateEvent(NULL, FALSE, FALSE, L"CallbackThreadS Event");
HANDLE FileAsync::mDecayCallbackFinish = ::CreateEvent(NULL, FALSE, TRUE, L"CallbackThreadF Event");
unsigned int FileAsync::mReferenceCounter = 0;
...
{
    mReferenceCounter++;
    printf("mReferenceCounter++ = %u\n", mReferenceCounter);
    if (mReferenceCounter == 1)
    {
        printf("Wait for mDecayCallbackFinish\n");
        ::WaitForSingleObject(mDecayCallbackFinish, INFINITE);
        printf("Done, reset events\n");
        ::ResetEvent(mDecayCallbackFinish);
        ::ResetEvent(mDecayCallbackStart);
        mCallbackThread = ::CreateThread(0, 0, CallbackDispatcher, nullptr, 0, 0);
    }
}
...
FileAsync::~FileAsync()
{
    ...
    mReferenceCounter--;
    printf("RefCounter-- = %u\n", mReferenceCounter);
    // If no other instances exist, give the callback thread signal to decay
    if (mReferenceCounter == 0)
    {
        printf("Shoot decay signal\n");
        ::SetEvent(mDecayCallbackStart);
    }
}
...
DWORD FileAsync::CallbackDispatcher(LPVOID param)
{
    ...
    // until master thread gives signal to decay
    while (mReferenceCounter > 0)
        ::WaitForSingleObjectEx(mDecayCallbackStart, INFINITE, true);

    printf("Quitting dispatcher\n");
    // Signal that current slave thread's decay is finished, new one can be created when needed
    ::SetEvent(mDecayCallbackFinish);
    return 0;
}
