...
printf("FileAsync::Write()1\n");
    ...
    ::iocb* cbs[1];
    cbs[0] = iocbPtr;
auto ctxAIO = AsyncQueueManager::GetInstance().GetQueueContext();
printf("FileAsync::Write()2, ctx = %lu\n", ctxAIO);
    int enqueueResult = ::io_submit(ctxAIO, 1, cbs);
    if (enqueueResult < 0)
    {
