...
bool FileAsync::Init()
{
    ...
    if (mEventFD == -1)
    {
        LOG_ERROR("Failed to acquire eventFD: %s", strerror(errno));
        Close();
        return false;
    }
...
void FileAsync::Close()
{
    if (IsOpened())
    {
        ...
        if (mEventFD && !AsyncQueueManager::GetInstance().DequeueJob(mEventFD))
...
void FileAsync::JobDispatcher(int eventsNo, int eventFD)
{
    ...
    for (::io_event* event = eventsPollBuffer; event < loopLimit; event ++)
    {
        ::iocb* ioPtr = reinterpret_cast<::iocb*>(event->obj);
        if (ioPtr && ioPtr->aio_resfd == eventFD
                && (ioPtr->aio_lio_opcode == IOCB_CMD_PWRITE || ioPtr->aio_lio_opcode == IOCB_CMD_PREAD))
