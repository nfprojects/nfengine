...
Window::Window()
{
    ...
    mWindow = XCreateWindow(mDisplay, mRoot, 0, 0, mWidth, mHeight, 1, CopyFromParent, InputOutput, CopyFromParent, CWColormap | CWEventMask, &xSetWAttrib);
    ...
    switch (event.type)
    {
        ...
        case ButtonPress:
        {
            if(event.button < 4) // 1-3 MBtns, 4-5 MWheel
                ...
                this->OnScroll(event.button * (-2) + 9); // btn==4 is UP, btn==5 is DOWN
            return 0;
        }
        case ButtonRelease:
        {
        ...
        case ConfigureNotify:
        {
            ...
            {
                this->mWidth = confEvent.width;
                this->mHeight = confEvent.height;
                this->OnResize(this->mWidth, this->mHeight);

                if (this->mResizeCallback)
                    this->mResizeCallback(this->mResizeCallbackUserData);
            }
            XFree(confEvent);
            return 0;
        }
