...
void Window::MouseDown(uint32 button, int x, int y)
{
    ...
    xcb_grab_pointer_reply(mConnection, grab, nullptr);
...
void Window::ProcessMessages()
{
    ...
    while ((event = xcb_poll_for_event(mConnection)))
    {
        switch (event->response_type & ~0x80)
        {
            case XCB_EXPOSE:
            {
                xcb_clear_area(mConnection, 0, mWindow, 0, 0, 0, 0);
            }
            ...
            case XCB_CLIENT_MESSAGE:
            {
                ...
                // close handling
                xcb_intern_atom_cookie_t c = xcb_intern_atom(mConnection, 0, 16, "WM_DELETE_WINDOW");
                xcb_intern_atom_reply_t* r = xcb_intern_atom_reply(mConnection, c, nullptr);

                if (static_cast<xcb_atom_t>(cm->data.data32[0]) == r->atom)
                {
                    this->Close();
                    this->OnClose();
                }
