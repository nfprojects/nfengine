...
{
    if (mNodes)
        NFE_GENERIC_FREE(mNodes);
    if (mObjects)
        NFE_GENERIC_FREE(mObjects);
    if (mIDs)
        NFE_GENERIC_FREE(mIDs);
}
...
bool PackedArray<ObjType, IDType, Alignment>::Resize(size_t newSize)
{
    ...
    ObjType* newObjects = static_cast<ObjType*>(NFE_GENERIC_MALLOC(sizeof(ObjType) * newSize, Alignment));
    ListNode* newNodes = static_cast<ListNode*>(NFE_GENERIC_MALLOC(sizeof(ListNode) * newSize, Alignment));
    IDType* newIDs = static_cast<IDType*>(NFE_GENERIC_MALLOC(sizeof(IDType) * newSize, Alignment));

    // allocation failed
    if (newNodes == nullptr || newObjects == nullptr || newIDs == nullptr)
    {
        NFE_GENERIC_FREE(newObjects);
        NFE_GENERIC_FREE(newNodes);
        NFE_GENERIC_FREE(newIDs);
        return false;
    }
