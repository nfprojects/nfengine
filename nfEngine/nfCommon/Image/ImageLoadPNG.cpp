...
void userReadData(png_structp pngPtr, png_bytep data, png_size_t length)
{
    ...
    InputStream* pStream = static_cast<InputStream*>(pData);

    if (pStream)
        pInfo->offset += pStream->Read(length, data);
}

bool Image::LoadPNG(InputStream* pStream)
{
    ...
    for (png_uint_32 i = 0; i < imgHeight; i++)
    {
        ...
        rowPtrs[i] = static_cast<png_bytep>(static_cast<uchar*>(data.get()) + q);
    }
    ...
    Mipmap mipmap(data.get(), imgWidth, imgHeight, dataSize);
    png_destroy_read_struct(&pngPtr, &infoPtr, pngInfoPPZero);
