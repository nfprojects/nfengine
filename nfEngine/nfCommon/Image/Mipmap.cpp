...
Color Mipmap::GetTexel(uint32 x, uint32 y, ImageFormat fmt) const
{
    switch (fmt)
    {
        ...
        case ImageFormat::RGB_UByte:
        {
            uchar* data = static_cast<uchar*>(mData.GetData());
            uchar colors[4];
            colors[0] = data[3 * (y * mWidth + x)];
            colors[1] = data[3 * (y * mWidth + x) + 1];
            colors[2] = data[3 * (y * mWidth + x) + 2];
            colors[3] = 255;
            return VectorLoadUChar4(colors);
        }
        ...
        case ImageFormat::RGBA_UByte:
        {
            uchar* data = static_cast<uchar*>(mData.GetData());
            uchar colors[4];
            colors[0] = data[4 * (y * mWidth + x)];
            colors[1] = data[4 * (y * mWidth + x) + 1];
            colors[2] = data[4 * (y * mWidth + x) + 2];
            colors[3] = data[4 * (y * mWidth + x) + 3];
            return VectorLoadUChar4(colors);
        }
        ...
        case ImageFormat::R_Float:
        {
            ...
            return Color(r, 0.0f, 0.0f, 255.0f);
        }
...
void Mipmap::SetTexel(const Color& v, uint32 x, uint32 y, ImageFormat fmt)
{
    switch (fmt)
    {
        ...
        {
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[y * mWidth + x] = static_cast<uchar>(v.f[3]);
            break;
        }
        ...
        case ImageFormat::R_UByte:
        {
            ...
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[y * mWidth + x] = static_cast<uchar>(v.f[0]);
            break;
        }
        ...
        case ImageFormat::RGB_UByte:
        {
            // TODO use function similar to VectorStoreUChar4
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[3 * (y * mWidth + x)] = static_cast<uchar>(v.f[0]);
            data[3 * (y * mWidth + x) + 1] = static_cast<uchar>(v.f[1]);
            data[3 * (y * mWidth + x) + 2] = static_cast<uchar>(v.f[2]);
            break;
        }
        ...
        {
            uchar* data = static_cast<uchar*>(mData.GetData());
            data = &data[4 * (y * mWidth + x)];
            VectorStoreUChar4(v, data);
            break;
        }
