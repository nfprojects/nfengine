...
Color Mipmap::GetTexel(uint32 x, uint32 y, ImageFormat fmt) const
{
    switch (fmt)
    {
        ...
        case ImageFormat::RGBA_UByte:
        {
            ...
            return Color(colors);*/
            return VectorLoadUChar4(data) * g_Byte2Float;
        }
...
void Mipmap::SetTexel(const Color& v, uint32 x, uint32 y, ImageFormat fmt)
{
    switch (fmt)
    {
        case ImageFormat::A_UByte:
        {
            ...
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[y * mWidth + x] = static_cast<uchar>(v.f[3] * 255.0f);
            break;
        }
        ...
        case ImageFormat::R_UByte:
        {
            // TODO use function similar to VectorStoreUChar4
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[y * mWidth + x] = static_cast<uchar>(v.f[0] * 255.0f);
            break;
        }
        ...
        case ImageFormat::RGB_UByte:
        {
            // TODO use function similar to VectorStoreUChar4
            uchar* data = static_cast<uchar*>(mData.GetData());
            data[3 * (y * mWidth + x)    ] = static_cast<uchar>(v.f[0] * 255.0f);
            data[3 * (y * mWidth + x) + 1] = static_cast<uchar>(v.f[1] * 255.0f);
            data[3 * (y * mWidth + x) + 2] = static_cast<uchar>(v.f[2] * 255.0f);
            break;
        }
        ...
        case ImageFormat::RGBA_UByte:
        {
            ...
            break;
            /*data[4 * (y * mWidth + x)    ] = static_cast<uchar>(v.f[0] * 255.0f);
            data[4 * (y * mWidth + x) + 1] = static_cast<uchar>(v.f[1] * 255.0f);
            data[4 * (y * mWidth + x) + 2] = static_cast<uchar>(v.f[2] * 255.0f);
            data[4 * (y * mWidth + x) + 3] = static_cast<uchar>(v.f[3] * 255.0f);
            break;*/
        }
