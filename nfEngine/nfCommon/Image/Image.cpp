...
bool Image::SetData(void* data, uint32 width, uint32 height, ImageFormat format)
{
    Release();
...
bool Image::DecompressDDS()
{
    ...
    const int size = static_cast<int>(mWidth * mHeight * BitsPerPixel(gDDSSrcFormat) / 8);
...
bool Image::GenerateMipmapsActual(MipmapFilter filterType, uint32 num)
{
    ...
    Mipmap* mipmap;
    Color (Mipmap::*filterFunctor)(uint32, uint32, ImageFormat) = nullptr;

    // Resolve what filter should be used
    switch (filterType)
    {
    case MipmapFilter::GammaCorrectedLinear:
        filterFunctor = &Mipmap::FilterGammaCorrected;
        break;

    case MipmapFilter::Box:
        filterFunctor = &Mipmap::FilterBox;
        break;
    ...
    for (uint32 i = 0; i < num; i++)
    {
        ...
        for (uint32 y = 0; y < nextMipHeight; y++)
        {
            ...
            {
                Color outputTexel = (mipmap->*filterFunctor)(x, y, mFormat);
...
bool Image::ConvertActual(ImageFormat destFormat)
{
    ...
    for (uint32 i = 0; i < mMipmaps.size(); i++)
    {
        ...
        newMipmaps[i].mData.resize(width * height * BitsPerPixel(destFormat) / 8);
