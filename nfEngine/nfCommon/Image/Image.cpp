...
int Image::SetData(void* pData, int width, int height, ImageFormat format)
{
    ...
    mHeight = height;
    mFormat = format;
    memcpy(mipmap.GetData(), pData, mipmap.GetDataSize());
    mMipmaps.push_back(mipmap);
...
int Image::Load(InputStream* pStream)
{
    ...
    auto loadLambda = [&](std::function<int(InputStream*)> loadFunc)
    {
        pStream->Seek(0);
...
void* Image::GetData(int mipmap) const
{
    Mipmap* mmap = GetMipmap(mipmap);
    if (mmap)
        return mmap->GetData();
...
const ImageMipmap& Image::GetMipmap(int id) const
{
    if (mMipmaps.empty())
        return nullptr;
...
int Image::GenerateMipmaps(uint32 num, int filterType)
{
    //empty image
    if (!GetData())
    {
    ...
    Mipmap* pMip = GetMipmap(0);

    for (uint32 i = 0; i < num; i++)
    {
        Mipmap nextMip;
        nextMip.SetWidth((pMip->width / 2) + (pMip->width % 2));
        nextMip.SetHeight((pMip->height / 2) + (pMip->height % 2));
        ...
        nextMip.SetDataSize(nextMip.width * nextMip.height * BitsPerPixel(mFormat) / 8);
        nextMip.AllocData();

        // NEEDS CHANGE - NO SUPPORT FOR non-power-of-2 !!!!
        uint32 nextMipHeight = nextMip.GetHeight();
        uint32 nextMipWidth = nextMip.GetWidth();
        MipmapFilter* filter = MipmapFilter::CreateFilter(0, pMip);
        for (uint32 y = 0; y < nextMipHeight; y++)
        {
            for (uint32 x = 0; x < nextMipWidth; x++)
            {
               ...
               /*
                Vector a = pMip.GetTexel(2 * x,     2 * y,     mFormat);
                Vector b = pMip.GetTexel(2 * x + 1, 2 * y,     mFormat);
                Vector c = pMip.GetTexel(2 * x,     2 * y + 1, mFormat);
                Vector d = pMip.GetTexel(2 * x + 1, 2 * y + 1, mFormat);

                nextMip.SetTexel(((a + b) + (c + d)) * 0.25f, x, y, mFormat);
                */
...
int Image::Convert(ImageFormat destFormat)
{
    ...
    ImageConverter converter(GetData(0), GetMipmap(0)->GetDataSize(), width, height);
    ...
    if (SetData(converter.GetOutputData(), mWidth, mHeight, destFormat))
    {
