...
bool ReadPixelsWithPalette(InputStream* stream, size_t offset, uint32 width, uint32 height,
                            uint8_t bitsPerPixel, std::vector<Mipmap> &dest, std::vector<RGBQuad> &palette);
    ...
    else
    {
        ...
        for (int y = static_cast<int>(height - 1); y >= 0; y--)
        {
            ...
            for (int x = 0; x < static_cast<int>(width); x += colorsPer4Bytes)
            {
                ...
                {
                    colorData32 = ((colorData[x / colorsPer4Bytes] >> (j * bitsPerPixel)) & colorsMask);
                    ...
                    for (uchar i = 0; i < 4; i++)
                    {
                        ...
                        // Store color data
                        imageData.get()[4 * (y * width + x + j) + i] = static_cast<uint8_t>(singleColor);
                    }
                            ...
                            uint8_t bitsPerPixel, std::vector<Mipmap> &dest, std::vector<RGBQuad> &palette)
{
    ...
    for (int y = static_cast<int>(height - 1); y >= 0; y--)
    {
        ...
        {
            int colorDataPos = static_cast<int>(floor(x / colorsPerByte));

            // Read all the pixels stored in loaded byte (there can be 1, 2, 4 or even 8)
            for (uint8_t i = 0; i < colorsPerByte; i++)
            {
                int index = (colorData[colorDataPos] >> (bitsPerPixel * i)) & bitMask;
