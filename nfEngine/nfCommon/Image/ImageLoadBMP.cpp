                ...
                uint8_t bitsPerPixel, std::vector<Mipmap> &dest, uint32_t* colorMask)
{
    ...
    uint32_t lineSize = width * colorSize;
    uint32_t lineSizeActual = lineSize;
    ...
    else
    {
        ...
        for (int y = static_cast<int>(height - 1); y >= 0; y--)
        {
            ...
            for (uint32_t x = 0; x < width; x += colorsPer4Bytes)
            {
                // Read 2 or 4 bytes, depending on 16 or 32 bpp
                for (uint32_t j = 0; j < colorsPer4Bytes; j++)
                {
                    uint32_t byteIndex = x / colorsPer4Bytes;

                    // Variable for storing single pixel data in 32bits format
                    uint32_t colorData32 = ((colorData[byteIndex] >> (j * bitsPerPixel)) & colorsMask);
                            ...
                            std::vector<RGBQuad> &palette)
{
    ...
    uint32_t lineSize = (width + (width % colorsPerByte)) / colorsPerByte;
    uint32_t lineSizeActual = lineSize;
    while (lineSizeActual % 4)
        lineSizeActual++;
    ...
    for (int y = static_cast<int>(height - 1); y >= 0; y--)
    {
        ...
        for (uint32_t x = 0; x < width; x++)
        {
            ...
            uint8_t bitShift = bitsPerPixel * (x % colorsPerByte);
