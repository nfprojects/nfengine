...
{
    Task* task = nullptr;
    TaskID currTaskID = NFE_INVALID_TASK_ID;
    size_t instanceId = 0;
    ...
    {
        {
            ...
            currTaskID = mTasksQueue.front();
            task = &mTasks[currTaskID];
            instanceId = task->mNextInstance++;
            if (task->mNextInstance == task->mInstancesNum)
                mTasksQueue.pop();
        }

        // execute
        task->mCallback(instanceId, thread->mId); // TODO
                              ...
                              TaskID parentID, TaskID dependencyID)
{
    ...
    task.mInstancesLeft = instancesNum;
    task.mInstancesNum = instancesNum;
