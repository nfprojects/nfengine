 ...
 */
NFE_ALIGN16
class NFCOMMON_API Matrix final
{
public:
    union
    {
    ...
    {
        Vector tmp0 = a.SplatX() * r[0];
        Vector tmp1 = a.SplatY() * r[1];
        Vector tmp2 = a.SplatZ() * r[2];
        return (tmp0 + tmp1) + (tmp2 + r[3]);
    }
