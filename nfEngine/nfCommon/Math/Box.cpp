...
Box TransformBox(const Matrix& matrix, const Box& localBox)
{
    ...
    float x = Vector::Dot3V(localBox.SupportVertex(matrix.r[0]) - boxCenter, matrix.r[0])[0];
    float y = Vector::Dot3V(localBox.SupportVertex(matrix.r[1]) - boxCenter, matrix.r[1])[0];
    float z = Vector::Dot3V(localBox.SupportVertex(matrix.r[2]) - boxCenter, matrix.r[2])[0];
