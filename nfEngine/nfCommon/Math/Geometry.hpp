...
struct Sphere
{
    ...
    NFE_INLINE Sphere(const Vector& origin, float r)
    {
        this->origin = origin;
        this->r = r;
    }
...
NFE_INLINE Box TransformBox(const Matrix& matrix, const Box& localBox)
{
    ...
    float x = VectorDot3(localBox.SupportVertex(matrix.r[0]) - boxCenter, matrix.r[0]);
    float y = VectorDot3(localBox.SupportVertex(matrix.r[1]) - boxCenter, matrix.r[1]);
    float z = VectorDot3(localBox.SupportVertex(matrix.r[2]) - boxCenter, matrix.r[2]);
