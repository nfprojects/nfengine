...
{
    Vector ab = p2 - p1;
    float dot = VectorDot3(p - p1, ab);
    float len_sq = VectorDot3(ab, ab);
...
bool Intersect(const Vector& point, const Frustum& frustum)
{
    ...
    {
        __m128 plane = frustum.planes[i];
        __m128 dot = _mm_dp_ps(point, plane, 0x71);
        dot = _mm_add_ss(dot, _mm_shuffle_ps(plane, plane, _MM_SHUFFLE(3, 3, 3, 3)));
        if (_mm_comilt_ss(dot, _mm_setzero_ps()))
            return false;
    }
...
bool Intersect(const Triangle& tri, const Frustum& frustum)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        int outside = _mm_comigt_ss(PlanePointDot3(frustum.planes[i], tri.v0), _mm_setzero_ps());
        outside &= _mm_comigt_ss(PlanePointDot3(frustum.planes[i], tri.v1), _mm_setzero_ps());
        outside &= _mm_comigt_ss(PlanePointDot3(frustum.planes[i], tri.v2), _mm_setzero_ps());
...
bool Intersect(const Vector& point, const Sphere& sphere)
{
    ...
    return _mm_comile_ss(VectorDot3(segment, segment), _mm_mul_ss(radiiSum, radiiSum));
}
...
bool Intersect(const Frustum& frustum, const Sphere& sphere)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (_mm_comilt_ss(dot, invRadius)) return false;
    }
    ...
    for (int i = 0; i < 8; ++i)
        if ((float)PlanePointDot3(plane, frustum.verticies[i]) < sphere.r)
            return true;
