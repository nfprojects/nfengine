...
void GameWindow::InitCamera()
{
    ...
    if (!mCameraEntity)
    {
        ...
        input->Enable();
        mCameraEntity->AddComponent(std::move(input));

        EntityControllerPtr controller(new FreeCameraController);
        mCameraEntity->AttachController(std::move(controller));
    }
...
void GameWindow::OnMouseDown(UINT button, int x, int y)
{
    ...
    if (button == 0)
    {
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::KeyPress, "Shoot"));
    }
    else if (button == 1)
    {
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::KeyPress, "CameraControl"));
    }
}
...
void GameWindow::OnMouseMove(int x, int y, int deltaX, int deltaY)
{
    ...
    if (deltaX != 0)
    {
        ...
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::Axis, "CameraYaw", value));
    }
    if (deltaY != 0)
    {
        const float value = sensitivity * static_cast<float>(deltaY);
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::Axis, "CameraPitch", value));
    }
}
...
void GameWindow::OnMouseUp(UINT button)
{
    ...
    if (button == 0)
    {
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::KeyRelease, "Shoot"));
    }
    else if (button == 1)
    {
        mScene->GetInputSystem()->PushEvent(Input::EventData(Input::EventData::Type::KeyRelease, "CameraControl"));
    }
}
