...
def main(argv):
    ...
    argParser = argparse.ArgumentParser(description='Test script for nfengine project.',
                                        epilog=parserEpilog, formatter_class=argparse.RawDescriptionHelpFormatter)
...
##    print args.single
##
##    singleArgsLen = len(args.single)
##    if singleArgsLen == 0:
##        loopLen = 1
##    else:
##        loopLen = singleArgsLen
##    for singleFilter in range(0, loopLen):
##        print 'filter = {}, looplen={}'.format(singleFilter, loopLen)
                ...
                if os.path.exists(xmlPath):
                    if args.verbose is not True:
                        ...
                        if len(testParser.failLinks) > 0:
                            if len(args.single) > 0:
                                fails.append(gatherFailed(_pathList, _testList, path, test, testParser.failLinks))
                            else:
                                fails.append(gatherFailed(pathList, testList, path, test, testParser.failLinks))
                    os.remove(xmlPath)
