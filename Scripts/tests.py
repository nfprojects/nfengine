...
def gatherFailed(paths, tests, path, test, failLinks):
    ...
    failString = '-s {};{} --args --gtest_filter='.format(testPos[0], pathPos[0])
...
def main(argv):
    ...
    argParser.add_argument('-s', '--single', default='', metavar='\'tNo;vNo\'',
                                help='Runs single gtest file. testNo and verNo are listed at the bottom.')
    ...
    if len(args.single) > 0:
        ...
        if semicolon < 0:
            print 'Wrong argument for \'--single\' option. Please input it as:'
            print '-s \'tNo;vNo\' (with \' \')'
            exit(1)
        if singleArg[0] == '\'':
            print 'Wrong argument for \'--single\' option. Please input it as:'
            print '-s tNo;vNo (without \' \')'
            exit(1)
