...
public:
    ...
    NFE_FORCE_INLINE Box_Simd8(const Box& box0, const Box& box1, const Box& box2, const Box& box3,
    ...
    NFE_FORCE_INLINE Box_Simd8(const Box& boxLo, const Box& boxHi)
    ...
    // splat single box
    NFE_FORCE_INLINE Box_Simd8(const Box& box)
        : min(box.min)
        , max(box.max)
    { }

    // build SIMD box from 8 boxes
    NFE_FORCE_INLINE explicit Box_Simd8(const Box* boxes)
