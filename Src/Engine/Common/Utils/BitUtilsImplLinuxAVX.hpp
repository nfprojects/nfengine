...
template<>
NFE_FORCE_INLINE uint16 BitUtils<uint16>::SwapEndianness(uint16 x)
{
    return __builtin_bswap16(x);
}

template<>
NFE_FORCE_INLINE uint32 BitUtils<uint32>::SwapEndianness(uint32 x)
{
    return __builtin_bswap32(x);
}

template<>
NFE_FORCE_INLINE uint64 BitUtils<uint64>::SwapEndianness(uint64 x)
{
